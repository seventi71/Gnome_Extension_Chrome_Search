# Search Provider - Gnome Search
This extension adds a Chrome search provider to Gnome Search
![Chrome Search Providers](/assets/providers.png)

# Installation:
- Download the zip file and unzip it.
  [Download] (https://github.com/seventi71/Search-Provider/archive/refs/heads/main.zip)
- Rename top folder, make sure the folder name is:
  ``SearchProvider@github.com``
- Copy the unzipped renamed folder to:
  ``~/.local/share/gnome-shell/extensions``
- Logout of Gnome and Login, then go to extension and enable.

# Usage Tip: 
Placement of search provider is at the bottom of the screen by default. To move it to the top, disable and re-enable search providers in Gnome search settings.
NOTE: The provider will only populate the first 6 options that are enabled.

When searching you get the option to:
- Ask Gemini, e.g. How to make a curry?
- Google Search, e.g. Reddit Best Linux Repo
- Search YouTube, e.g. Focus Music
- Search Maps, e.g. Directions to "location"
- Search News, e.g. Local or "topic"
- Search Translate, e.g. Hello, there stranger
- Open Link, e.g Localhost:8000

# Credits:
This extension was created using Zed and Inkling and is open source.
Some of the code was reused from the extension 'Browser Search Provider'
Some of the preferences code was reused from 'Toggle touchpad on or off'

This code lives on github at 'https://github.com/seventi71/Search-Provider'
